...
FROM Actor_Pelicula AP JOIN Actor A
WHERE AP.ap_id_actor != 
                        (
                            SELECT actor_id FROM actor WHERE actor_nombre = "UMA"
                            AND actor_apellido = "WOOD"
                        )  -- Indicamos en el conjunto A que debe estar UMA WOOD 
AND AP.ap_id_pelicula NOT IN 
                        (
                            SELECT AP1.ap_id_pelicula
                            FROM Actor_Pelicula AP1
                            WHERE AP1.ap_id_actor = 
                            (
                                SELECT actor_id FROM actor 
                                WHERE actor_nombre =  "UMA" 
                                AND actor_apellido = "WOOD"
                            )                            
                        );

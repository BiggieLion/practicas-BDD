...
/*Insertamos los datos en la tabla (Modificar de acuerdo al nombre de la base de datos donde
se encuentre la base de datos "actor pelicula")*/
...
-- Relacion de actores y peliculas
...
SELECT ap_id_pelicula, COUNT(ap_id_actor)
FROM Actor_Pelicula
GROUP BY ap_id_pelicula HAVING COUNT(ap_id_actor) = 1;

/*MODIFICACION 4: Listar el top 5 de actores con mas peliculas*/
SELECT actor_nombre AS Nombre, actor_apellido AS Apellido, COUNT(ap_id_pelicula)
FROM Actor_Pelicula JOIN Actor JOIN Pelicula
GROUP BY Nombre, Apellido
HAVING COUNT(ap_id_pelicula) = 
                                (
                                    SELECT MAX(ap_id_pelicula)
                                    FROM
                                    (
